...
export const FrontId: React.FC<ComponentProp> = ({ session, ...props }) => {
  ...
  return (
    <div>
      {!photo && <Modal.Root variant="fully" isOpen={step === 0}>
        <Modal.Header className="text-center">
          <h3 className="heading-3">Tome una fotografía clara, sin sombras ni reflejos.</h3>
        </Modal.Header>
        <Modal.Body className="flex items-center">
          <video></video>
        </Modal.Body>
        <Modal.Footer>
          <button className="button is-primary" onClick={takePhoto}>Continuar</button>
        </Modal.Footer>
      </Modal.Root>}
      ...
      {photo && (
        <>
          <Modal.Root isOpen={Boolean(photo) && step === 1} variant="fully">
            ...
            <Modal.Footer>
              <button onClick={uploadFrontId} className="button is-primary">Continuar</button>
              <button onClick={takePhoto} className="button">Volver a Capturar</button>
            </Modal.Footer>
          </Modal.Root>

          <Modal.Root isOpen={Boolean(photo) && step === 2} variant="fully">
            <Modal.Body className="flex items-center">
              <img  className={status} src={`data:image/${photo.format};base64,${photo.base64String}`}></img>
            </Modal.Body>
            <Modal.Footer className="gap-6">
              ...
              {error && <>
                ...
                <button onClick={tryAgain} className="button is-primary">Capturar otra vez</button>
              </>}
            </Modal.Footer>
          </Modal.Root>
        </>
